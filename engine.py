...
def _arbitrate_candidates(candidates: Dict[str, int], htf_bias: int) -> Tuple[int, str, List[str], int]:
    if not candidates:
        return 0, "NONE", [], 0
    ...
    if len(tied) == 1:
        ...
        if len(aligned) == 1:
            ...
            return 0, "NONE", codes, penalty - 2
    strategy = pick[0].replace("_LONG", "").replace("_SHORT", "")
    return pick[1], strategy, codes, penalty
...
def _arbitrate_candidates(candidates: Dict[str, int]) -> Tuple[int, str, List[str], int]:
    ...
    if len(tied) == 1:
        ...
        signs = {1 if v > 0 else -1 for _, v in tied}
        if len(signs) == 1:
            pick = sorted(
                tied,
                key=lambda kv: STRATEGY_PRECEDENCE.get(kv[0].replace("_LONG", "").replace("_SHORT", ""), 99),
            )[0]
            codes.append("ARBITRATION_CONSENSUS_TIE")
        else:
            aligned = [(k, v) for k, v in tied if (v > 0 and htf_bias > 0) or (v < 0 and htf_bias < 0)]
            if len(aligned) == 1:
                pick = aligned[0]
...
def _tier_and_action(score: int, blockers: List[str], tf_cfg: dict) -> tuple[str, str]:
    if blockers:
        return "NO-TRADE", "SKIP"
    if score >= tf_cfg["trade_long"] or score <= tf_cfg["trade_short"]:
        return "A+", "TRADE"
    if score >= tf_cfg["watch_long"] or score <= tf_cfg["watch_short"]:
...
def compute_score(
    ...
    trace: Dict[str, object] = {"trace_version": 1, "degraded": [], "blockers": [], "candidates": {}, "strategy": "NONE", "net": 0}
    ...
    detector_pts, strategy, arb_codes, arb_penalty = _arbitrate_candidates(candidates, htf_bias)
    ...
    group_totals: Dict[str, int] = {"policy": 0, "macro": 0, "market": 0}
    group_caps: Dict[str, int] = {"policy": 3, "macro": 3, "market": 4}
    reasons, codes, degraded, blockers = [], [], [], []
    breakdown = {"trend_alignment": 0, "momentum": 0, "volatility": 0, "volume": 0, "htf": 0, "penalty": 0}
        ...
        for group_name, kws in GROUPS.items():
            for keyword, weight in kws.items():
                if keyword in text:
                    ...
                    if count < 2:
                        raw = int(weight * (2 if count == 0 else 1))
                        cap = group_caps[group_name]
                        current = group_totals[group_name]
                        if raw > 0:
                            contribution = max(0, min(raw, cap - current))
                        elif raw < 0:
                            contribution = min(0, max(raw, -cap - current))
                        else:
                            contribution = 0
                        if contribution != 0:
                            breakdown["momentum"] += contribution
                            group_totals[group_name] += contribution
                            codes.append(f"NEWS_{keyword.replace(' ', '_').upper()}")
                        contribution = int(weight * (2 if count == 0 else 1))
                        breakdown["momentum"] += contribution
                        codes.append(f"NEWS_{keyword.replace(' ', '_').upper()}")
    ...
    direction = "LONG" if score >= tf_cfg["watch_long"] else "SHORT" if score <= tf_cfg["watch_short"] else "NEUTRAL"
    regime = "long_signal" if score >= tf_cfg["trade_long"] else "short_signal" if score <= tf_cfg["trade_short"] else f"{reg}_bias"
    ...
    tier, action = _tier_and_action(score, blockers, tf_cfg)
